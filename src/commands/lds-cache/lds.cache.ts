...
function getTargetPath(source: string, path: string): string {
  if (path.startsWith('./')) return join(source, path.slice(2));
  ...
  async handler(args) {
    ...
    for (const layer of args.layers) {
      ...
      if (layerVersion != null) {
        ...
        await fsa.head(fsa.toUrl(source)); // Ensure we have read permission for the source

        logger.info({ layerId, layerVersion, source }, 'Collection:Item:Fetch');
        const fileName = `./${layerId}_${layerVersion}.gpkg`;
        const targetLocation = join(args.target, fileName);
        await fsa.write(fsa.toUrl(targetLocation), fsa.readStream(fsa.toUrl(source)).pipe(createGunzip()));
      }

      const collectionJson = await fsa.readJson<stac.StacCollection>(
        fsa.toUrl(`s3://linz-lds-cache/${layerId}/collection.json`),
      );
      ...
      const targetFile = join(args.target, lastItem.href.replace('.json', '.gpkg'));
      ...
      await fsa.write(fsa.toUrl(targetFile), fsa.readStream(fsa.toUrl(targetPath)).pipe(createGunzip()));
    }
  },
});
