...
export const topoStacValidation = command({
  ...
  args: {
    ...
    includeLatest: flag({
      type: boolean,
      long: 'include-latest',
      description: 'Include the validate for latest topo stacs',
    }),
  },
  async handler(args) {
    ...
    if (args.includeLatest) {
      const base = args.input.href.endsWith('/')
        ? args.input.href.slice(0, args.input.href.length - 1)
        : args.input.href;
      inputs.push(new URL(`${base}-latest/`));
    }
