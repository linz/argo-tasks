...
export const commandMapSheetCoverage = command({
  ...
  async handler(args) {
    ...
    const filesToProcess = createFileList(mapSheets, true);

    const fileListPath = new URL('file-list.json', args.output);
    await fsa.write(urlToString(fileListPath), JSON.stringify(filesToProcess));
    logger.info(
      {
        duration: performance.now() - startTime,
        layersFound: allLayers.features.length,
        layersNeeded: captureDates.features.length,
        itemsToCreate: filesToProcess.length,
      },
